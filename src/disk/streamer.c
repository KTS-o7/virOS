...
{
    int sector = stream->pos / viros_SECTOR_SIZE;
    int offset = stream->pos % viros_SECTOR_SIZE;
    int total_to_read = total;
    bool overflow = (offset + total_to_read) >= viros_SECTOR_SIZE;
    char buf[viros_SECTOR_SIZE];

    if (overflow)
    {
        total_to_read -= (offset + total_to_read) - viros_SECTOR_SIZE;
    }
