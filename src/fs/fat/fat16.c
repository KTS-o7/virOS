...
#define viros_FAT16_SIGNATURE 0x29
#define viros_FAT16_FAT_ENTRY_SIZE 0X02
#define viros_FAT16_BAD_SECTOR 0XFF7
#define viros_FAT16_UNUSED 0X00
...
int fat16_get_total_items_for_directory(struct disk *disk, uint32_t directory_start_sector)
{
    ...
    if (diskstreamer_seek(stream, directory_start_pos) != viros_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
    while (1)
    {
        if (diskstreamer_read(stream, &item, sizeof(item)) != viros_ALL_OK)
        {
            res = -EIO;
            goto out;
        }
                             ...
                             struct fat_directory *directory)
{
    ...
    struct disk_stream *stream = fat_private->directory_stream;
    if (diskstreamer_seek(stream, fat16_sector_to_absolute(disk, root_dir_sector_pos)) != viros_ALL_OK)
    {
        res = -EIO;
        goto out;
    }

    if (diskstreamer_read(stream, dir, root_dir_size) != viros_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
...
int fat16_resolve(struct disk *disk)
{
    ...
    if (diskstreamer_read(stream, &fat_private->header, sizeof(fat_private->header)) != viros_ALL_OK)
    {
        res = -EIO;
        goto out;
    }

    if (fat_private->header.shared.extended_header.signature != viros_FAT16_SIGNATURE)
    {
        res = -EFSNOTUS;
        goto out;
    }

    if (fat16_get_root_directory(disk, fat_private, &fat_private->root_directory) != viros_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
...
static int fat16_get_fat_entry(struct disk *disk, int cluster)
{
    ...
    res = diskstreamer_seek(stream, fat_table_position * (cluster * viros_FAT16_FAT_ENTRY_SIZE));
    if (res < 0)
    {
        goto out;
    }
...
static int fat16_get_cluster_for_offset(struct disk *disk, int starting_cluster, int offset)
{
    ...
    for (int i = 0; i < clusters_ahead; i++)
    {
        ...
        if (entry == viros_FAT16_BAD_SECTOR)
        {
            res = -EIO;
            goto out;
        }
                                              ...
                                              const char *filename)
{
    ...
    char tmp_filename[viros_MAX_PATH];
