...
struct filesystem *filesystems[viros_MAX_FILESYSTEMS];
struct file_descriptor *file_descriptors[viros_MAX_FILE_DESCRIPTORS];

// Get a free file descriptor
static struct filesystem **fs_get_free_filesystem()
{
    int i = 0;
    for (i = 0; i < viros_MAX_FILESYSTEMS; i++)
    {
        if (filesystems[i] == 0)
        {
...
{
    int res = -ENOMEM;
    for (int i = 0; i < viros_MAX_FILE_DESCRIPTORS; i++)
    {
...
{
    if (fd <= 0 || fd > viros_MAX_FILE_DESCRIPTORS)
    {
        return 0;
    }
...
struct filesystem *fs_resolve(struct disk *disk)
{
    ...
    for (int i = 0; i < viros_MAX_FILESYSTEMS; i++)
    {
