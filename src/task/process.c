...
static struct process *processes[viros_MAX_PROCESSES] = {};
...
{
    if (process_id < 0 || process_id >= viros_MAX_PROCESSES)
    {
        return NULL;
    }
...
{
    int res = -ENOMEM;
    for (int i = 0; i < viros_MAX_PROGRAM_ALLOCATIONS; i++)
    {
...
{
    for (int i = 0; i < viros_MAX_PROCESSES; i++)
    {
        if (processes[i])
        {
...
int process_map_binary(struct process *process)
{
    ...
    paging_map_to(process->task->page_directory,
                  (void *)viros_PROGRAM_VIRTUAL_ADDRESS, process->ptr,
...
int process_map_memory(struct process *process)
{
    ...
    paging_map_to(process->task->page_directory,
                  (void *)viros_PROGRAM_VIRTUAL_STACK_ADDRESS_END,
                  process->stack,
                  paging_align_address(process->stack + viros_USER_PROGRAM_STACK_SIZE),
                  PAGING_IS_PRESENT | PAGING_ACCESS_FROM_ALL | PAGING_IS_WRITEABLE);
...
int process_load_for_slot(const char *filename, struct process **process, int process_slot)
{
    ...
    program_stack_ptr = kzalloc(viros_USER_PROGRAM_STACK_SIZE);
